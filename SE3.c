...
int main() {
    ...
    char materias_nombres[3][30] = {"Mate", "Costos", "Progra"};

    for (int cont1 = 0; cont1 < 5; cont1++)
    {
        printf("Nombre del estudiante %d: ", cont1 + 1);
        fflush(stdin);
        fgets(estudiantes_nombres[cont1], 30, stdin);
        len = strlen(estudiantes_nombres[cont1]) - 1;
        estudiantes_nombres[cont1][len] = '\0';
    }
    for (int cont1 = 0; cont1 < estudiantes; cont1++) {
        for (int cont2 = 0; cont2 < materias; cont2++) {
            do {
                printf("Calificacion de %s en %s (0-10): ", estudiantes_nombres[cont1], materias_nombres[cont2]);
                fflush(stdin);
